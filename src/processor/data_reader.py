...
processing_pipeline.add_component(
...
processing_pipeline.add_component("retriever", InMemoryEmbeddingRetriever(document_store=document_store))
...
processing_pipeline.add_component("prompt_builder", PromptBuilder(template=template))
...
#         api_params={"url": "https://p2pi0qga0rbpiqyv.us-east-1.aws.endpoints.huggingface.cloud"},
#         token=Secret.from_token(os.environ["HF_API_TOKEN"]),
#     ),
# )

# After embedding the query, connect it to the document retriever
processing_pipeline.connect("embedder.embedding", "retriever.query_embedding")

# Connect the collected documents from the retriever to the prompt builder
processing_pipeline.connect("retriever", "prompt_builder.documents")

# Connect the built prompt to the language model generator
processing_pipeline.connect("prompt_builder", "llm")
